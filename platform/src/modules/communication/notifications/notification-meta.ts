 ...
 */
import { Bell, AlertTriangle, ArrowRightLeft, FlaskConical, Calendar, Pill, ClipboardList, Activity, Stethoscope, MessageSquare } from '@/components/icons/lucide';
...
export const NOTIFICATION_META: Record<NotificationType, NotificationMeta> = {
  ...
  message: { icon: MessageSquare, color: 'var(--accent-primary)', bg: 'var(--accent-light)', label: 'Messages' },
};

/** Order the source filters are presented in — urgency first, admin last. */
export const NOTIFICATION_TYPE_ORDER: NotificationType[] = [
  'alert', 'triage', 'visit', 'lab', 'transfer', 'referral', 'appointment', 'message', 'progress', 'prescription',
];
