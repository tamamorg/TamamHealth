...
import { NATIONAL_PROJECTION_EXCLUDED_TYPES } from '../index';
/**
 * Analytics-domain implementation of the CouchDB → PostgreSQL Sync Webhook
 *
...
const DB_TABLE_MAP: Record<string, string> = {
  ...
  tamamhealth_medication_administrations: 'medication_administrations',
...
function resolveTable(db: string, doc?: Record<string, unknown>): string | undefined {
  ...
  if (typeof doc?.type === 'string' && NATIONAL_PROJECTION_EXCLUDED_TYPES.has(doc.type)) return undefined;
...
const FIELD_MAPPERS: Record<string, FieldMapper> = {
  ...
  medication_administrations: (doc) => ({
    id: doc._id,
    prescription_id: doc.prescriptionId,
    patient_id: doc.patientId,
    patient_name: doc.patientName,
    ...
    status: doc.status,
    dose_given: doc.doseGiven,
    route: doc.route,
    administered_by: doc.administeredBy,
    administered_by_name: doc.administeredByName,
    witness_id: doc.witnessId,
    witness_name: doc.witnessName,
    reason: doc.reason,
    notes: doc.notes,
    voids_administration_id: doc.voidsAdministrationId,
    created_at: doc.createdAt,
    updated_at: doc.updatedAt,
  }),
      ...
      try {
        if (change.deleted) {
          ...
          if (resolved === undefined && typeof (change.doc as { type?: unknown })?.type === 'string'
              && NATIONAL_PROJECTION_EXCLUDED_TYPES.has((change.doc as { type: string }).type)) {
            processed++;
            lastSeq = change.seq;
            continue;
          }
          ...
          await upsertDocument(table, cleaned);
        }
        processed++;
        lastSeq = change.seq;
      } catch (err) {
