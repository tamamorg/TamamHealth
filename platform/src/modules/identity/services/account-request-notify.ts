...
import type { AccountRequestDoc, UserDoc } from '@/lib/db-types';
import { buildAppUrl } from '@/modules/identity/provisioning/user-invite';
import { INVITE_TTL_HOURS } from '@/modules/identity/provisioning/user-invite';
...
export async function notifyRequestSubmitted(
  ...
  try {
    ...
    if (!verifyUrl) return;
    const { sendAccountRequestVerifyEmail } = await import('@/modules/identity/email/account-request-verify');
...
export async function notifyApproversOfRequest(doc: AccountRequestDoc): Promise<number> {
  try {
    ...
    const { sendAccountRequestAlertEmail } = await import('@/modules/identity/email/account-request-alert');
