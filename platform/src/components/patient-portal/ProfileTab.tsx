        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div className="pp-card">
            ...
            <div style={{ padding: '11px 14px' }}>
              <b style={{ display: 'block', fontSize: 13.5, fontWeight: 600, color: 'var(--text-primary)' }}>
                {patient.nokName || '—'}{patient.nokRelationship ? ` · ${patient.nokRelationship}` : ''}
              </b>
              <span style={{ display: 'block', marginTop: 2, fontSize: 12, color: 'var(--text-muted)' }}>{patient.nokPhone || '—'}</span>
            </div>
          </div>
          ...
          <div className="pp-card pp-card--danger">
            ...
            <div className="pp-pillbox">
              {realAllergies.length > 0 ? realAllergies.map((a, i) => (
                ...
                <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{t('patientPortal.noKnownAllergies')}</span>
              )}
            </div>
          </div>
          ...
          <div className="pp-card">
            ...
            <div className="pp-pillbox">
              {realConditions.length > 0 ? realConditions.map((c, i) => (
                ...
                <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{t('patientPortal.noChronicConditions')}</span>
              )}
            </div>
          </div>
        </div>
