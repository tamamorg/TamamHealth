...
// `patientName` below is reconciled against the deterministic generated
// roster (`data/mock.ts` via `buildPoolIdentity`/`normalizePoolIdentity`) at
// write time in `safePut`, but the literals here are kept in sync with that
// roster too — a stale literal is otherwise invisible in this file and still
// leaks to any reader that imports `labOrders` directly instead of going
// through the seeded database (see `getDemoLabResultsByPatient` in
// `patient-portal-demo.ts`).
