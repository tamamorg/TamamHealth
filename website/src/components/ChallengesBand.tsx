...
export default function ChallengesBand() {
  ...
  return (
    <div style={{ background: "#113055", padding: "56px 52px 52px" }} className="tm-pad-lg">
      <h2 style={{ fontSize: "clamp(22px, 3vw, 32px)", margin: "0 0 8px", color: "#FFFFFF" }}>{t("What paper costs a facility, every single day")}</h2>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: "18px 24px", marginBottom: 28, flexWrap: "wrap" }}>
        <p style={{ margin: 0, maxWidth: 640, fontSize: 15.5, lineHeight: 1.65, color: "rgba(255,255,255,0.78)" }}>
          {t("Open any failure for what it costs today and the steps that replace it.")}
        </p>
        <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
          <button type="button" onClick={() => scroll(-1)} aria-label={t("Previous")} className="tm-chalstep" style={{ width: 42, height: 42, border: "1px solid rgba(255,255,255,0.4)", background: "transparent", color: "#FFFFFF", cursor: "pointer", fontSize: 16, lineHeight: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>‹</button>
          <button type="button" onClick={() => scroll(1)} aria-label={t("Next")} className="tm-chalstep-lit" style={{ width: 42, height: 42, border: "1px solid #7CC7FF", background: "#7CC7FF", color: "#113055", cursor: "pointer", fontSize: 16, lineHeight: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>›</button>
        </div>
      </div>
        ...
        {CHALLENGES.map((c) => (
          <Link key={c.slug} href={`/challenges/${c.slug}`} className="blueprint tm-chalcard" style={{ flex: "0 0 316px", display: "flex", flexDirection: "column", textDecoration: "none", color: "#FFFFFF", borderColor: "rgba(255,255,255,0.28)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 9, padding: "20px 22px", flex: 1 }}>
              ...
              <span className="fs125" style={{ marginTop: "auto", paddingTop: 12, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "#7CC7FF" }}>{t("Learn more →")}</span>
            </div>
          </Link>
        ))}
      </div>
