    ...
    <section id="footprint" style={{ padding: "90px 32px 80px", background: "#FFFFFF" }}>
      <div style={{ maxWidth: 1320, margin: "0 auto" }}>
        <div style={{ textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: 18 }}>
          <h2 style={{ fontSize: "clamp(27px, 4vw, 44px)", margin: 0, maxWidth: 760 }}>Starting in Juba, built to scale across sub-Saharan Africa</h2>
          ...
          <p style={{ margin: 0, maxWidth: 640, fontSize: 15.5, lineHeight: 1.65, color: "var(--color-neutral-700)" }}>
            ...
            that offline-first digital records can work in the hardest conditions, and a foundation built to scale across sub-Saharan Africa.
          </p>
        </div>
        <div className="tm-map tm-split" style={{ position: "relative", marginTop: 34, display: "grid", gridTemplateColumns: "400px 1fr", alignItems: "center", gap: 0 }}>
          ...
          <div className="blueprint tm-site-card" style={{ background: "var(--color-surface)", padding: "34px 32px 30px", zIndex: 2 }}>
            ...
            <div className="tm-desk-steppers" style={{ display: "flex", gap: 8 }}>
              <button className="btn btn-secondary btn-icon" onClick={() => select(site.index - 1)} aria-label="Previous site">‹</button>
              <button className="btn btn-secondary btn-icon" onClick={() => select(site.index + 1)} aria-label="Next site">›</button>
            </div>
          </div>
          <iframe
            ref={frameRef}
            src="/map-south-sudan.html"
            title="Map of South Sudan showing pilot and planned sites"
            style={{ width: "100%", height: 560, border: 0, marginLeft: -60 }}
          />
        </div>
        <HeroNav
          items={[]}
